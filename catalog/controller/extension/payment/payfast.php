...
class ControllerExtensionPaymentPayfast extends Controller {
    ...
    public function index() {
         ...
         * get token first
         */
        $merchantid = $this->config->get('payfast_merchant_id');
        $securitykey = $this->config->get('payfast_security_key');
        ...
        $data['customer_mobile'] = $order_info['telephone'];
    ...
    public function callback() {
        ...
        if ($order_info) {
            ...
            if ($apps_statuscode == "000") {
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payfast_pending_status_id'), $comment_message, true);
                if ($redirect == "Y") {
            ...
            switch ($apps_statuscode) {
                case "529":
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payfast_canceled_status_id'), $comment_message, true);
                    break;
                default:
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payfast_failed_status_id'), $comment_message, true);
                    break;
            }
    ...
    private function payfastCurlRequest($url) {
		...
		curl_setopt($ch,CURLOPT_USERAGENT,'OpenCart 2 APPS PayFast Plugin');
